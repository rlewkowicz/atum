...
// Main configures and runs the Atum operator process.
func Main() {
	...
	if err := (&controller.PlatformIdentityConfigurationReconciler{
	...
	if err := manager.AddHealthzCheck("healthz", healthz.Ping); err != nil {
		ctrl.Log.Error(err, "health check")
		os.Exit(1)
	}
	if err := manager.AddReadyzCheck("readyz", healthz.Ping); err != nil {
		ctrl.Log.Error(err, "ready check")
		os.Exit(1)
	}
